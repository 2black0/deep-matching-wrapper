...
import onnx
from clidd_onnx_utils import ONNX_DIR, make_input, run_onnx
...
def main():
    ...
    args=ap.parse_args()
    models=[Path(m) for m in args.models] if args.models else sorted(ONNX_DIR.glob('clidd_*x*.onnx'))
    ...
    for m in models:
        ...
        try:
            ...
            sess,y=run_onnx(m,image,cuda=True)
            ...
            item['onnx_ok']=False; item['error']=repr(e)
        out.append(item)
