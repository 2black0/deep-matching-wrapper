...
// Correct preprocessing: pad to multiple of 32
...
static std::pair<std::vector<int64_t>, std::vector<int64_t>> matchFeaturesCPU(
    ...
    std::vector<float> expDist(dist.size());
    for (size_t idx = 0; idx < dist.size(); ++idx) {
        expDist[idx] = std::exp((dist[idx] - 1.0f) * beta);
    }    
    ...
    for (int i = 0; i < n1; ++i) {
        const float* row = expDist.data() + i * n2;
        float rowSum = 0.0f;
    ...
    for (int i = 0; i < n1; ++i) {
        float* simRow = sim.data() + i * n2;
        const float* expRow = expDist.data() + i * n2;
        float rowSum = sum1[i];
        
        for (int j = 0; j < n2; ++j) {
            float val = expRow[j];
            float similarity = (val * val) / (rowSum * sum2[j] + 1e-12f);
            simRow[j] = similarity;
        }
    }    
